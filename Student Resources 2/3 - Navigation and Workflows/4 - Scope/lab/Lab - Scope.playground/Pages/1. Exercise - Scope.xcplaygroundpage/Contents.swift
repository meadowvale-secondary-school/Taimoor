...
//Since foo is a local scope since itnexists only inside the function. So, if you called it outside the function then Swift will not find foo and result in a compiler error.
/*:
...
//The variable x is now outside the function making it a global scope which means it can be accessed throughout the program.
/*:
...
greeting(greeting: "Good Morning", name: "Bob")
greeting(greeting: nil, name: "Jack")
/*:
