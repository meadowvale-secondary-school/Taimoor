...
func add (x:Int?, y:Int?) -> Int? {
    guard let valueOfx = x,
        let valueOfy = y else {return nil}
    let sum = valueOfx + valueOfy
    return sum
}
add(x: 20, y: 20)
add(x: 2, y: nil)
...
func createUser() -> User? {
    ...
    let userName = User(firstName: firstName, lastName: lastName, age: age)
    return userName
}
...
createUser()
let user = createUser()!
print("The first name of the user is \(user.firstName), last name is \(user.lastName) and the user is \(user.age) years old.")
