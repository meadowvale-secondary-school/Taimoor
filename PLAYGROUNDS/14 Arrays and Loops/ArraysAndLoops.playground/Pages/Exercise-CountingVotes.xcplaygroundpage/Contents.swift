...
for counting in shouldMascotChangeVotes {
    if counting == true {
...
for counting in shouldInstallCoffeeVendingMachineVotes {
    if counting == true {
        countsYes2 = countsYes2 + 1
    }
    else {
        countsNo2 = countsNo2 + 1
    }
}
for counting in shouldHaveMorePollOptionsVotes {
    if counting == true {
        countsYes3 = countsYes3 + 1
    }
    else {
        countsNo3 = countsNo3 + 1
    }
}
...
func voting (issue: String, votes: Array<Bool> ) {
    var False = 0
    var True = 0
    for addingUp in votes {
        if addingUp == true {
            True = True + 1
            print(True)
        }
